...
def insert_sorted(srt_lnk_lst, elem):
    """ mutates a sorted linked list by inserting element into its correct place."""
    left = srt_lnk_lst.first_node()
    right = left.next
    stop = False
    while stop == False:
        if ((left.data is None) or (elem > left.data)) and ((elem < right.data) or (right.data is None)):
            new = srt_lnk_lst.Node(elem, left, right)
            left.next = new
            right.prev = new
            stop = True
        left = left.next
